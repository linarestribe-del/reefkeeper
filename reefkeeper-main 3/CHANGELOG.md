## v4.3.19 — Build 2F.2: Observer Image Overlay Fix

- Fixes the disconnected-image placeholder remaining visible over a successfully loaded Aquarium Observer image.
- Adds an explicit `[hidden]` display rule for Observer images, placeholders, and timestamp labels.
- Bumps the stylesheet cache key so the corrected layout loads immediately after deployment.
- Adds regression coverage for the loaded-image overlay state.
