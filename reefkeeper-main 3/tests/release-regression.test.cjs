...
assert(html.includes('css/app.css?v=20260720-build-2f2-observer-overlay-fix'));
...
assert(html.includes('observer.js?v=20260720-build-2f2-observer-overlay-fix'));
...
assert(css.includes('Build 2F.2: hide the Observer fallback layer after a real image loads.'));
assert(css.includes('.observer-image-placeholder[hidden]'));
...
assert.equal(pkg.version, '4.3.19');
