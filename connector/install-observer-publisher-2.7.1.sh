...
EXPECTED_PUBLISHER_SHA256="0a1372ed110ae37d0b76702688436e7dd86dd2094007d1692236e53fcaa29ccf"
...
TMP_DIR="$(mktemp -d /tmp/reefkeeper-observer-2.7.1.XXXXXX)"
trap 'rm -rf "$TMP_DIR"' EXIT

echo "REEF KEEPER OBSERVER PUBLISHER 2.7.1 INSTALLER"
echo "================================================"
...
BACKUP="${TARGET}.before-2.7.1-${STAMP}.bak"
CALIBRATOR_BACKUP=""
FILTER_CALIBRATOR_BACKUP=""

if [ -f "$CALIBRATOR_TARGET" ]; then
  CALIBRATOR_BACKUP="${CALIBRATOR_TARGET}.before-2.7.1-${STAMP}.bak"
fi
if [ -f "$FILTER_CALIBRATOR_TARGET" ]; then
  FILTER_CALIBRATOR_BACKUP="${FILTER_CALIBRATOR_TARGET}.before-2.7.1-${STAMP}.bak"
fi

echo "Active publisher file: $TARGET"
echo "Downloading verified Publisher 2.7.1 and calibration helpers..."
...
for filename, digest_expected in expected.items():
    url = f'{base}/connector/{filename}?v=4.3.49-9c1'
    request = urllib.request.Request(url, headers={
        'User-Agent': 'ReefKeeper-Observer-Updater/2.7.1',
...
if "PUBLISHER_VERSION = '2.7.1'" not in (output / 'observer-publisher.py').read_text(encoding='utf-8'):
    raise SystemExit('Downloaded publisher is not version 2.7.1.')
...
echo "Installing Publisher 2.7.1..."
...
PY
  ...
  if [ "$RESULT" = "success" ] && [ "$EXIT_CODE" = "0" ] && \
     [ "$CONTROLLED_VERSION" = "2.7.1" ] && [ "$CONTROLLED_OK" = "True" ] && \
...
if [ "$FINAL_RESULT" != "success" ] || [ "$FINAL_EXIT" != "0" ] || \
   [ "$FINAL_VERSION" != "2.7.1" ] || [ "$FINAL_OK" != "True" ] || \
...
echo
echo "UPDATE RESULT: PASS — PUBLISHER 2.7.1 ACTIVE"
