...
EXPECTED_PUBLISHER_SHA256="caee0bace73a682581e223b1beac9a042bc121dcb3f45e3f87aeed193c241f4d"
TMP_DIR="$(mktemp -d /tmp/reefkeeper-observer-2.8.3.XXXXXX)"
trap 'rm -rf "$TMP_DIR"' EXIT

echo "REEF KEEPER OBSERVER PUBLISHER 2.8.3 INSTALLER"
echo "================================================"
...
url = f'{base}/connector/observer-publisher.py?v=4.3.69-9m'
request = urllib.request.Request(url, headers={
    'User-Agent': 'ReefKeeper-Observer-Updater/2.8.3',
...
if "PUBLISHER_VERSION = '2.8.3'" not in text:
    raise SystemExit('Downloaded publisher is not version 2.8.3.')
if 'tracking_paused' not in text or 'allow_offline' not in text:
    raise SystemExit('Downloaded publisher does not include the 9M return water-level reliability path.')
...
echo "Installing Publisher 2.8.3..."
...
if [ "$RESULT" != "success" ] || [ "$EXIT_CODE" != "0" ] || [ "$VERSION" != "2.8.3" ] || [ "$OK" != "True" ] || [ "$AFTER_HASH" = "$BEFORE_HASH" ]; then
...
echo
echo "UPDATE RESULT: PASS — PUBLISHER 2.8.3 ACTIVE"
