...
EXPECTED_PUBLISHER_SHA256="b8109bdafc8d89a69e36c6e37628acdfd4090136dcacd64c228477b3b91ba150"
EXPECTED_TRAINER_SHA256="4df82851b4f936d93acf7538fba125ec0b20651b6a050aca5c224a4ecfd77b48"
TMP_DIR="$(mktemp -d /tmp/reefkeeper-water-level-learning-1.1.XXXXXX)"
trap 'rm -rf "$TMP_DIR"' EXIT

echo "REEF KEEPER 9N.1 FEED-AWARE WATER-LEVEL INSTALLER"
echo "===================================================="
...
PUBLISHER_BACKUP="${TARGET}.before-2.8.5-9n1-${STAMP}.bak"
TRAINER_BACKUP="${TRAINER_TARGET}.before-9n1-${STAMP}.bak"
...
echo "Downloading verified 9N.1 files..."
...
files = [
    ('observer-publisher.py', expected_publisher, "PUBLISHER_VERSION = '2.8.5'", 'baseline_nearest_edge'),
    ('return-water-level-trainer.py', expected_trainer, "TRAINER_VERSION = '1.0.1'", 'likelyFeedHighSampleCount'),
]
for name, expected, marker, feature in files:
    url = f'{base}/connector/{name}?v=4.3.71-9n1'
    request = urllib.request.Request(url, headers={
        'User-Agent': 'ReefKeeper-9N1-FeedAware-Water-Level/1.1',
...
echo "Installing Publisher 2.8.5 and feed-aware water-level trainer..."
...
if [ "$RESULT" != "success" ] || [ "$EXIT_CODE" != "0" ] || [ "$VERSION" != "2.8.5" ] || [ "$OK" != "True" ] || [ "$AFTER_HASH" = "$BEFORE_HASH" ]; then
...
echo
echo "UPDATE RESULT: PASS — 9N.1 FEED-AWARE WATER-LEVEL INSTALLED"
