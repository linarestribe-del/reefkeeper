...
TMP_DIR="$(mktemp -d /tmp/reefkeeper-observer-2.5.XXXXXX)"
trap 'rm -rf "$TMP_DIR"' EXIT

echo "REEF KEEPER OBSERVER PUBLISHER 2.5 INSTALLER"
echo "================================================"
...
if [ -f "$CALIBRATOR_TARGET" ]; then
  CALIBRATOR_BACKUP="${CALIBRATOR_TARGET}.before-2.5-${STAMP}.bak"
fi

echo "Active publisher file: $TARGET"
echo "Downloading verified Publisher 2.5 and dual-camera calibration helper..."
...
for filename, digest_expected in expected.items():
    ...
    request = urllib.request.Request(url, headers={
        'User-Agent': 'ReefKeeper-Observer-Updater/2.5',
...
if "PUBLISHER_VERSION = '2.5'" not in (output / 'observer-publisher.py').read_text(encoding='utf-8'):
    raise SystemExit('Downloaded publisher is not version 2.5.')
...
if [ "$RESULT" != "success" ] || [ "$EXIT_CODE" != "0" ] || \
   [ "$CONTROLLED_VERSION" != "2.5" ] || [ "$CONTROLLED_OK" != "True" ] || \
...
if [ "$FINAL_RESULT" != "success" ] || [ "$FINAL_EXIT" != "0" ] || \
   [ "$FINAL_VERSION" != "2.5" ] || [ "$FINAL_OK" != "True" ] || \
...
echo
echo "UPDATE RESULT: PASS — PUBLISHER 2.5 ACTIVE"
