...
EXPECTED_PUBLISHER_SHA256="cd0abaa67bcf74c29b533b428d4e3796e969bcf081852e3bce7fcadd3d68a552"
...
TMP_DIR="$(mktemp -d /tmp/reefkeeper-observer-2.8.1.XXXXXX)"
trap 'rm -rf "$TMP_DIR"' EXIT

echo "REEF KEEPER OBSERVER PUBLISHER 2.8.1 INSTALLER"
echo "================================================"
...
BACKUP="${TARGET}.before-2.8.1-${STAMP}.bak"
CALIBRATOR_BACKUP=""
FILTER_CALIBRATOR_BACKUP=""
FILTER_CONFIG_BACKUP=""

if [ -f "$CALIBRATOR_TARGET" ]; then
  CALIBRATOR_BACKUP="${CALIBRATOR_TARGET}.before-2.8.1-${STAMP}.bak"
fi
if [ -f "$FILTER_CALIBRATOR_TARGET" ]; then
  FILTER_CALIBRATOR_BACKUP="${FILTER_CALIBRATOR_TARGET}.before-2.8.1-${STAMP}.bak"
fi
if [ -f "$FILTER_CONFIG" ]; then
  FILTER_CONFIG_BACKUP="${FILTER_CONFIG}.before-2.8.1-${STAMP}.bak"
fi

echo "Active publisher file: $TARGET"
echo "Downloading verified Publisher 2.8.1 and calibration helpers..."
...
for filename, digest_expected in expected.items():
    url = f'{base}/connector/{filename}?v=4.3.60-9i'
    request = urllib.request.Request(url, headers={
        'User-Agent': 'ReefKeeper-Observer-Updater/2.8.1',
...
if "PUBLISHER_VERSION = '2.8.1'" not in (output / 'observer-publisher.py').read_text(encoding='utf-8'):
    raise SystemExit('Downloaded publisher is not version 2.8.1.')
...
echo "Installing Publisher 2.8.1..."
...
PY
  ...
  if [ "$RESULT" = "success" ] && [ "$EXIT_CODE" = "0" ] && \
     [ "$CONTROLLED_VERSION" = "2.8.1" ] && [ "$CONTROLLED_OK" = "True" ] && \
...
if [ "$FINAL_RESULT" != "success" ] || [ "$FINAL_EXIT" != "0" ] || \
   [ "$FINAL_VERSION" != "2.8.1" ] || [ "$FINAL_OK" != "True" ] || \
...
echo
echo "UPDATE RESULT: PASS — PUBLISHER 2.8.1 ACTIVE"
