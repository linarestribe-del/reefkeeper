...
EXPECTED_PUBLISHER_SHA256="4ddf347224f669751939137f17825dcbe172d67e974d48d3ff17176de5aeb100"
...
TMP_DIR="$(mktemp -d /tmp/reefkeeper-observer-2.7.2.XXXXXX)"
trap 'rm -rf "$TMP_DIR"' EXIT

echo "REEF KEEPER OBSERVER PUBLISHER 2.7.2 INSTALLER"
echo "================================================"
...
BACKUP="${TARGET}.before-2.7.2-${STAMP}.bak"
CALIBRATOR_BACKUP=""
FILTER_CALIBRATOR_BACKUP=""

if [ -f "$CALIBRATOR_TARGET" ]; then
  CALIBRATOR_BACKUP="${CALIBRATOR_TARGET}.before-2.7.2-${STAMP}.bak"
fi
if [ -f "$FILTER_CALIBRATOR_TARGET" ]; then
  FILTER_CALIBRATOR_BACKUP="${FILTER_CALIBRATOR_TARGET}.before-2.7.2-${STAMP}.bak"
fi

echo "Active publisher file: $TARGET"
echo "Downloading verified Publisher 2.7.2 and calibration helpers..."
...
for filename, digest_expected in expected.items():
    url = f'{base}/connector/{filename}?v=4.3.50-9c2'
    request = urllib.request.Request(url, headers={
        'User-Agent': 'ReefKeeper-Observer-Updater/2.7.2',
...
if "PUBLISHER_VERSION = '2.7.2'" not in (output / 'observer-publisher.py').read_text(encoding='utf-8'):
    raise SystemExit('Downloaded publisher is not version 2.7.2.')
...
echo "Installing Publisher 2.7.2..."
...
PY
  ...
  if [ "$RESULT" = "success" ] && [ "$EXIT_CODE" = "0" ] && \
     [ "$CONTROLLED_VERSION" = "2.7.2" ] && [ "$CONTROLLED_OK" = "True" ] && \
...
if [ "$FINAL_RESULT" != "success" ] || [ "$FINAL_EXIT" != "0" ] || \
   [ "$FINAL_VERSION" != "2.7.2" ] || [ "$FINAL_OK" != "True" ] || \
...
echo
echo "UPDATE RESULT: PASS — PUBLISHER 2.7.2 ACTIVE"
