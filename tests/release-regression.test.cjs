...
assert(html.includes('Reef Keeper v4.3.73 Maintenance 9O'));
...
assert(fs.existsSync('connector/install-observer-publisher-2.8.0.sh'));
...
assert(fs.existsSync('tests/observer-9m-return-water-level-reliability.test.mjs'));
assert(fs.existsSync('connector/install-return-water-level-learning-1.1.sh'));
assert(fs.existsSync('connector/return-water-level-trainer.py'));
assert(fs.existsSync('tests/observer-9n-return-water-level-learning.test.mjs'));
assert(fs.existsSync('tests/observer-9n2-filter-roll-replacement-cycle.test.mjs'));
assert(filterRollUi.includes('New cycle started — physical baseline'));
assert(filterRollUi.includes('older rejected camera diagnostics are collapsed once a newer camera reading is accepted'));
assert(filterRollUi.includes('Camera diagnostics ('));
...
assert(fs.readFileSync('connector/observer-publisher.py', 'utf8').includes("PUBLISHER_VERSION = '2.8.5'"));
