...
assert(html.includes('css/app.css?v=20260722-maintenance-8a-r2-storage'));
...
assert(html.includes('app.js?v=20260722-maintenance-8a-r2-storage'));
assert(html.includes('observer.js?v=20260722-maintenance-8a-r2-storage'));
...
assert.equal(pkg.version, '4.3.43');
...
for (const file of ['MAINTENANCE_8A_RELEASE_MANIFEST.md', 'MAINTENANCE_8A_TEST_REPORT.md', 'checksums/maintenance-8A.sha256']) {
  assert(fs.existsSync(file), `Maintenance 8A release file is missing: ${file}`);
}
...
assert(html.includes('Reef Keeper v4.3.43 Maintenance 8A'));
