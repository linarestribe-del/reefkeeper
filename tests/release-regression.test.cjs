...
assert(html.includes('css/app.css?v=20260722-maintenance-7b-status-canvas'));
...
assert(html.includes('app.js?v=20260722-maintenance-7b-status-canvas'));
...
assert.equal(pkg.version, '4.3.42');
...
for (const file of ['MAINTENANCE_7B_RELEASE_MANIFEST.md', 'MAINTENANCE_7B_TEST_REPORT.md', 'checksums/maintenance-7B.sha256']) {
  assert(fs.existsSync(file), `Maintenance 7B release file is missing: ${file}`);
}
...
assert(html.includes('Reef Keeper v4.3.42 Maintenance 7B'));
