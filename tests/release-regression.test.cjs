...
assert(html.includes('css/app.css?v=20260722-maintenance-7a-mobile-ui'));
...
assert(html.includes('app.js?v=20260722-maintenance-7a-mobile-ui'));
...
assert.equal(pkg.version, '4.3.41');
...
for (const file of ['MAINTENANCE_7A_RELEASE_MANIFEST.md', 'MAINTENANCE_7A_TEST_REPORT.md', 'checksums/maintenance-7A.sha256']) {
  assert(fs.existsSync(file), `Maintenance 7A release file is missing: ${file}`);
}
...
assert(html.includes('Reef Keeper v4.3.41 Maintenance 7A'));
