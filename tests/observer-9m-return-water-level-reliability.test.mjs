...
assert.match(publisher, /PUBLISHER_VERSION = '2\.8\.3'/, 'Publisher version should be 2.8.3.');
...
assert.match(index, /Reef Keeper v4\.3\.69 Maintenance 9M/, 'App settings version should show 9M.');
assert.match(index, /Water-level tracking is optional; return timelapse support is active\./, 'Return chamber tools should describe the new optional water-level mode.');
