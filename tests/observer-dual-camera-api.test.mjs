...
try {
  ...
  await publishHandler({ method: 'POST', headers, query: {}, body: {
    ...
    health: { status: 'healthy', issues: [] }, filterRoll: { enabled: true, configured: true, available: true, measuredAt: '2026-07-24T01:00:00Z', confidence: 0.81, apparentOuterRadius: 120, apparentCoreRadius: null, schedule: { hoursLocal:[9,15,21], measurementsPerDay:3, minSpacingMinutes:240 } }, historyImages: []
  } }, res);
  ...
  assert.equal(res.payload.filterRoll.apparentCoreRadius, null);
