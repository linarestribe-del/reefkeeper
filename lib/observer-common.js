...
function normalizeObserverFilterRoll(value) {
  ...
  return {
    ...
    message: cleanObserverString(source.message || '', 240),
    note: cleanObserverString(source.note || '', 240),
    measuredAt,
    measurementId: cleanObserverString(source.measurementId || measuredAt || '', 140),
    sourceImageId: cleanObserverString(source.sourceImageId || '', 140),
    ...
    schedule: {
      ...
      measurementsPerDay: Math.max(1, Math.min(6, Math.floor(observerNumber(schedule.measurementsPerDay || source.measurementsPerDay) || hours.length || 3))),
