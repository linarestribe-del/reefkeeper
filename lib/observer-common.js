...
function normalizeObserverFilterRoll(value) {
  ...
  const remainingPct = observerNumber(source.remainingPct);
  const outerRadius = observerNumber(source.apparentOuterRadius);
  const coreRadius = observerNumber(source.apparentCoreRadius);
  const thicknessPct = observerNumber(source.apparentThicknessPct);
  return {
