// Reef Keeper Maintenance 8D — dual-camera Observer validation, monitoring, health, alerts, and timelapses

import { timingSafeEqual } from 'node:crypto';

export const OBSERVER_SCHEMA_VERSION = 10;
export const OBSERVER_STATUS_PATH = 'aquarium-observer/status.json';
export const OBSERVER_CAMERA_IDS = Object.freeze({
  overview: 'overview',
  return: 'return'
});
export const OBSERVER_RETURN_IMAGE_SLOTS = Object.freeze({
  latest: 'aquarium-observer/return-chamber/latest.jpg'
});
...
export const OBSERVER_RETURN_TIMELAPSE_SLOTS = Object.freeze({
...
function observerPublicMediaBaseUrl() {
  const raw = String(
    process.env.REEF_OBSERVER_PUBLIC_MEDIA_BASE_URL ||
    process.env.REEF_OBSERVER_PUBLIC_BASE_URL ||
    ''
  ).trim();
  if (!raw) return '';
  try {
...
function observerPublicObjectUrl(key) {
  const base = observerPublicMediaBaseUrl();
  if (!base) return '';
...
function observerMediaUrlForKey(key, fallbackUrl) {
  return observerPublicObjectUrl(key) || fallbackUrl;
}
...
function normalizeObserverTimelapseRecord(slot, value, cameraId = 'overview') {
  const item = value && typeof value === 'object' ? value : {};
  const normalizedSlot = normalizeObserverTimelapseSlot(slot);
  const camera = normalizeObserverCameraId(cameraId) || 'overview';
  ...
  const slots = camera === 'return' ? OBSERVER_RETURN_TIMELAPSE_SLOTS : OBSERVER_TIMELAPSE_SLOTS;
  const cameraQuery = camera === 'return' ? '&camera=return' : '';
  const fallbackVideoUrl = normalizedSlot ? `${OBSERVER_TIMELAPSE_VIDEO_ROUTE}&slot=${encodeURIComponent(normalizedSlot)}${cameraQuery}` : '';
  const videoUrl = normalizedSlot ? observerMediaUrlForKey(slots[normalizedSlot], fallbackVideoUrl) : '';
  return {
    ...
    videoUrl: available ? videoUrl : ''
  };
}

function normalizeCameraTimelapses(source, cameraId) {
  const timelapses = source && typeof source === 'object' && source.timelapses && typeof source.timelapses === 'object'
    ? source.timelapses
    : (source && typeof source === 'object' ? source : {});
  return {
    week: normalizeObserverTimelapseRecord('week', timelapses.week, cameraId),
    month: normalizeObserverTimelapseRecord('month', timelapses.month, cameraId)
  };
}

export function normalizeObserverTimelapseFeed(value) {
  const source = value && typeof value === 'object' ? value : {};
  const overviewSource = source.cameras?.overview || { timelapses: source.timelapses || source };
  const returnSource = source.cameras?.return || {};
  const overviewTimelapses = normalizeCameraTimelapses(overviewSource, 'overview');
  const returnTimelapses = normalizeCameraTimelapses(returnSource, 'return');
  return {
    schemaVersion: 2,
    ok: source.ok !== false,
...
export function normalizeObserverCameraId(value) {
  const camera = cleanObserverString(value || 'overview', 20).toLowerCase();
  return Object.prototype.hasOwnProperty.call(OBSERVER_CAMERA_IDS, camera) ? camera : null;
}

export function observerCameraImageUrl(cameraId = 'overview', slot = 'latest') {
  const camera = normalizeObserverCameraId(cameraId);
  const normalizedSlot = normalizeObserverSlot(slot);
  if (!camera || !normalizedSlot) return '';
  if (camera === 'return' && normalizedSlot !== 'latest') return '';
  const key = camera === 'return' ? OBSERVER_RETURN_IMAGE_SLOTS.latest : OBSERVER_IMAGE_SLOTS[normalizedSlot];
  if (camera === 'overview') {
    const fallback = normalizedSlot === 'latest'
      ? OBSERVER_IMAGE_ROUTE
      : `${OBSERVER_IMAGE_ROUTE}?slot=${encodeURIComponent(normalizedSlot)}`;
    return observerMediaUrlForKey(key, fallback);
  }
  return observerMediaUrlForKey(key, `${OBSERVER_IMAGE_ROUTE}?camera=return&slot=latest`);
}
...
export function normalizeObserverChangeAlert(value, index = 0) {
  ...
  return {
    ...
    source: {
      kind,
      issueCode,
      cameraId: cleanObserverString(item.source?.cameraId || item.cameraId || 'overview', 20),
      currentCapturedAt,
      previousCapturedAt
    }
  };
...
function normalizeComparisonRecord(slot, value) {
  ...
  return {
    ...
    imageUrl: available ? observerCameraImageUrl('overview', slot) : ''
  };
}
...
function normalizeObserverFilterRoll(value) {
  const source = value && typeof value === 'object' ? value : {};
  const schedule = source.schedule && typeof source.schedule === 'object' ? source.schedule : {};
  const roi = Array.isArray(source.roi) && source.roi.length === 4
    ? source.roi.map(item => Math.max(0, Math.min(1, observerNumber(item) || 0)))
    : null;
  const hours = Array.isArray(schedule.hoursLocal)
    ? schedule.hoursLocal.map(item => Math.max(0, Math.min(23, Math.floor(observerNumber(item) || 0)))).slice(0, 6)
    : [];
  const nullableNumber = value => value === null || value === undefined || value === '' ? null : observerNumber(value);
  const normalizeAttempt = value => {
    const item = value && typeof value === 'object' ? value : null;
    if (!item) return null;
    const measuredAt = observerIso(item.measuredAt || item.attemptedAt || item.captureAt);
    const outerRadius = nullableNumber(item.apparentOuterRadius);
    const remainingPct = nullableNumber(item.remainingPct);
    if (!measuredAt && outerRadius === null && remainingPct === null) return null;
    return {
      accepted: item.accepted === true || item.available === true,
      available: item.available === true,
      cameraId: cleanObserverString(item.cameraId || 'overview', 20),
      state: cleanObserverString(item.state || 'pending', 40),
      status: cleanObserverString(item.status || item.state || 'pending', 40),
      measurementId: cleanObserverString(item.measurementId || measuredAt || '', 180),
      sourceImageId: cleanObserverString(item.sourceImageId || '', 180),
      attemptedAt: observerIso(item.attemptedAt || item.measuredAt),
      measuredAt,
      confidence: Math.max(0, Math.min(1, observerNumber(item.confidence) || 0)),
      remainingPct: remainingPct === null ? null : Math.max(0, Math.min(100, remainingPct)),
      apparentOuterRadius: outerRadius === null ? null : Math.max(0, outerRadius),
      apparentCoreRadius: nullableNumber(item.apparentCoreRadius),
      apparentThicknessPct: nullableNumber(item.apparentThicknessPct),
      frameCount: Math.max(0, Math.floor(observerNumber(item.frameCount) || 0)),
      successfulFrameCount: Math.max(0, Math.floor(observerNumber(item.successfulFrameCount) || 0)),
      radiusDeviationPx: Math.max(0, observerNumber(item.radiusDeviationPx) || 0),
      analysisMessage: cleanObserverString(item.analysisMessage || item.message || '', 300),
      rejectionReason: cleanObserverString(item.rejectionReason || '', 300),
      note: cleanObserverString(item.note || '', 240),
      referenceOnly: item.referenceOnly === true
    };
  };
  const measuredAt = observerIso(source.measuredAt || source.captureAt);
  const remainingPct = nullableNumber(source.remainingPct);
  const outerRadius = nullableNumber(source.apparentOuterRadius);
  const coreRadius = nullableNumber(source.apparentCoreRadius);
  const thicknessPct = nullableNumber(source.apparentThicknessPct);
  const history = Array.isArray(source.attemptHistory)
    ? source.attemptHistory.slice(-12).map(normalizeAttempt).filter(Boolean)
    : [];
  return {
    enabled: source.enabled !== false,
    configured: source.configured === true,
    available: source.available === true,
    cameraId: cleanObserverString(source.cameraId || 'overview', 20),
    state: cleanObserverString(source.state || 'pending', 40),
    status: cleanObserverString(source.status || source.state || 'pending', 40),
    message: cleanObserverString(source.message || '', 300),
    analysisMessage: cleanObserverString(source.analysisMessage || '', 300),
    rejectionReason: cleanObserverString(source.rejectionReason || '', 300),
    note: cleanObserverString(source.note || '', 240),
    measuredAt,
    measurementId: cleanObserverString(source.measurementId || measuredAt || '', 180),
    sourceImageId: cleanObserverString(source.sourceImageId || '', 180),
    confidence: Math.max(0, Math.min(1, observerNumber(source.confidence) || 0)),
    remainingPct: remainingPct === null ? null : Math.max(0, Math.min(100, remainingPct)),
    apparentOuterRadius: outerRadius === null ? null : Math.max(0, outerRadius),
    apparentCoreRadius: coreRadius === null ? null : Math.max(0, coreRadius),
    apparentThicknessPct: thicknessPct === null ? null : Math.max(0, Math.min(100, thicknessPct)),
    frameCount: Math.max(0, Math.floor(observerNumber(source.frameCount) || 0)),
    successfulFrameCount: Math.max(0, Math.floor(observerNumber(source.successfulFrameCount) || 0)),
    radiusDeviationPx: Math.max(0, observerNumber(source.radiusDeviationPx) || 0),
    referenceOnly: source.referenceOnly === true,
    lastAccepted: normalizeAttempt(source.lastAccepted),
    lastAttempt: normalizeAttempt(source.lastAttempt),
    attemptHistory: history,
    roi,
    schedule: {
      hoursLocal: [...new Set(hours)],
      measurementsPerDay: Math.max(1, Math.min(6, Math.floor(observerNumber(schedule.measurementsPerDay || source.measurementsPerDay) || hours.length || 2))),
      minSpacingMinutes: Math.max(30, Math.min(1440, Math.floor(observerNumber(schedule.minSpacingMinutes) || 240)))
    }
  };
}
...
export function normalizeObserverHealth(value) {
  ...
  const localMonitoringSource = source.localMonitoring && typeof source.localMonitoring === 'object' ? source.localMonitoring : {};
  const localMonitoringBase = normalizeObserverHealthComponent(localMonitoringSource, 'Waiting for local visual monitoring data.');
  const imageQualityBase = normalizeObserverHealthComponent(localMonitoringSource.imageQuality, 'Waiting for image-quality analysis.');
  const sceneBase = normalizeObserverHealthComponent(localMonitoringSource.scene, 'Learning the stable sump view.');
  const waterLevelBase = normalizeObserverHealthComponent(localMonitoringSource.waterLevel, 'Water-level monitoring is not calibrated.');
  ...
  return {
    ...
    localMonitoring: {
      ...localMonitoringBase,
      enabled: localMonitoringSource.enabled !== false,
      evaluatedAt: observerIso(localMonitoringSource.evaluatedAt),
      mode: cleanObserverString(localMonitoringSource.mode || 'unknown', 20),
      imageQuality: {
        ...imageQualityBase,
        meanBrightness: Math.max(0, Math.min(255, observerNumber(localMonitoringSource.imageQuality?.meanBrightness) || 0)),
        contrast: Math.max(0, observerNumber(localMonitoringSource.imageQuality?.contrast) || 0),
        edgeEnergy: Math.max(0, observerNumber(localMonitoringSource.imageQuality?.edgeEnergy) || 0),
        darkFraction: Math.max(0, Math.min(1, observerNumber(localMonitoringSource.imageQuality?.darkFraction) || 0)),
        brightFraction: Math.max(0, Math.min(1, observerNumber(localMonitoringSource.imageQuality?.brightFraction) || 0)),
        obstructionStreak: Math.max(0, Math.floor(observerNumber(localMonitoringSource.imageQuality?.obstructionStreak) || 0))
      },
      scene: {
        ...sceneBase,
        baselineReady: observerBoolean(localMonitoringSource.scene?.baselineReady),
        changeScore: Math.max(0, Math.min(1, observerNumber(localMonitoringSource.scene?.changeScore) || 0)),
        anchorChangeScore: Math.max(0, Math.min(1, observerNumber(localMonitoringSource.scene?.anchorChangeScore ?? localMonitoringSource.scene?.changeScore) || 0)),
        fullChangeScore: Math.max(0, Math.min(1, observerNumber(localMonitoringSource.scene?.fullChangeScore ?? localMonitoringSource.scene?.changeScore) || 0)),
        unshiftedScore: Math.max(0, Math.min(1, observerNumber(localMonitoringSource.scene?.unshiftedScore) || 0)),
        shiftX: Math.trunc(observerNumber(localMonitoringSource.scene?.shiftX) || 0),
        shiftY: Math.trunc(observerNumber(localMonitoringSource.scene?.shiftY) || 0),
        movementLikely: observerBoolean(localMonitoringSource.scene?.movementLikely),
        maintenanceVariation: observerBoolean(localMonitoringSource.scene?.maintenanceVariation),
        learningState: cleanObserverString(localMonitoringSource.scene?.learningState || 'unknown', 40),
        anchorZones: Array.isArray(localMonitoringSource.scene?.anchorZones) ? localMonitoringSource.scene.anchorZones.slice(0, 8) : [],
        streak: Math.max(0, Math.floor(observerNumber(localMonitoringSource.scene?.streak) || 0))
      },
      waterLevel: {
        ...waterLevelBase,
        configured: observerBoolean(localMonitoringSource.waterLevel?.configured),
        confidence: Math.max(0, Math.min(1, observerNumber(localMonitoringSource.waterLevel?.confidence) || 0)),
        baselineYPercent: Math.max(0, Math.min(100, observerNumber(localMonitoringSource.waterLevel?.baselineYPercent) || 0)),
        currentYPercent: Math.max(0, Math.min(100, observerNumber(localMonitoringSource.waterLevel?.currentYPercent) || 0)),
        deltaPercent: Math.max(-100, Math.min(100, observerNumber(localMonitoringSource.waterLevel?.deltaPercent) || 0)),
        direction: cleanObserverString(localMonitoringSource.waterLevel?.direction || 'unknown', 20),
        streak: Math.max(0, Math.floor(observerNumber(localMonitoringSource.waterLevel?.streak) || 0))
      }
    },
    archive: {
      ...archiveBase,
...
export function normalizeObserverCameraStatus(cameraId, input, overrides = {}) {
  const camera = normalizeObserverCameraId(cameraId);
  if (!camera) throw new Error('Unknown Observer camera.');
  ...
  const publishedAt = observerIso(overrides.publishedAt || body.publishedAt || body.receivedAt);
  ...
  const defaultLabel = camera === 'return' ? 'Return chamber' : 'Sump overview';
  const defaultStream = camera === 'return' ? 'stream1' : 'stream1';
  const configured = body.configured === true || Boolean(capturedAt || publishedAt || imageAvailable || body.cameraLabel);
  return {
    cameraId: camera,
    configured,
    ...
    capturedAt,
    cameraLabel: cleanObserverString(body.cameraLabel || body.camera?.label || defaultLabel, 80),
    stream: cleanObserverString(body.stream || body.camera?.stream || defaultStream, 30),
    resolution: cleanObserverString(body.resolution || body.camera?.resolution || '2560×1440', 40),
    ...
    thumbnailUrl: imageAvailable ? observerCameraImageUrl(camera, 'latest') : '',
    comparisons: camera === 'overview' ? normalizeObserverComparisons(overrides.comparisons || body.comparisons) : normalizeObserverComparisons({}),
    filterRoll: camera === 'overview' ? normalizeObserverFilterRoll(overrides.filterRoll || body.filterRoll) : normalizeObserverFilterRoll({ enabled:false, configured:false, cameraId:'return' }),
    health,
    storage: {
...
export function normalizeObserverStatus(input, overrides = {}) {
  ...
  const health = normalizeObserverHealth(overrides.health || body.health);
  const publishedAt = observerIso(overrides.publishedAt || body.publishedAt) || new Date().toISOString();
  ...
  const base = {
    ...
    publishedAt,
    capturedAt,
    cameraLabel: cleanObserverString(body.cameraLabel || body.camera?.label || 'Sump overview', 80),
    stream: cleanObserverString(body.stream || body.camera?.stream || 'stream1', 30),
    resolution: cleanObserverString(body.resolution || body.camera?.resolution || '2560×1440', 40),
    captureIntervalMinutes: Math.max(1, Math.min(1440, observerNumber(body.captureIntervalMinutes || body.intervalMinutes) || 5)),
    publisherVersion: cleanObserverString(body.publisherVersion || health.publisher.version || '', 40),
    sizeBytes: Math.max(0, observerNumber(overrides.sizeBytes ?? body.sizeBytes ?? body.size_bytes) || 0),
    ...
    imageAvailable,
    imageVersion: cleanObserverString(overrides.imageVersion || body.imageVersion || capturedAt || '', 100),
    thumbnailUrl: imageAvailable ? observerCameraImageUrl('overview', 'latest') : '',
    comparisons,
    filterRoll: normalizeObserverFilterRoll(body.filterRoll),
    health,
    storage: {
      label: cleanObserverString(body.storage?.label || body.storageLabel || 'Local Pi drive', 80),
      totalBytes: Math.max(0, observerNumber(body.storage?.totalBytes) || 0),
  ...
  const cameraSource = body.cameras && typeof body.cameras === 'object' ? body.cameras : {};
  const overviewInput = cameraSource.overview && typeof cameraSource.overview === 'object'
    ? { ...base, ...cameraSource.overview, health: cameraSource.overview.health || base.health, storage: { ...base.storage, ...(cameraSource.overview.storage || {}) } }
    : base;
  const returnInput = cameraSource.return && typeof cameraSource.return === 'object' ? cameraSource.return : {};
  base.cameras = {
    overview: normalizeObserverCameraStatus('overview', overviewInput, {
      ok: overviewInput.ok,
      capturedAt: overviewInput.capturedAt,
      publishedAt: overviewInput.publishedAt,
      imageAvailable: overviewInput.imageAvailable === true,
      imageVersion: overviewInput.imageVersion,
      sizeBytes: overviewInput.sizeBytes,
      comparisons: overviewInput.comparisons,
      filterRoll: overviewInput.filterRoll,
      health: overviewInput.health
    }),
    return: normalizeObserverCameraStatus('return', returnInput, {
      ok: returnInput.ok,
      capturedAt: returnInput.capturedAt,
      publishedAt: returnInput.publishedAt,
      imageAvailable: returnInput.imageAvailable === true,
      imageVersion: returnInput.imageVersion,
      sizeBytes: returnInput.sizeBytes,
      health: returnInput.health
    })
  };
  return base;
}
...
export function awaitingObserverStatus(message = 'The app-side Aquarium Observer is ready, but the Raspberry Pi publishing bridge has not uploaded a capture yet.') {
  ...
  pending.cameras = {
    overview: normalizeObserverCameraStatus('overview', pending),
    return: normalizeObserverCameraStatus('return', { configured: false, ok: false, cameraLabel: 'Return chamber' })
  };
  return pending;
}
