// Reef Keeper Maintenance 8D — private Cloudflare R2 storage for dual-camera Observer media and metadata

import { createHash, createHmac } from 'node:crypto';
import { Readable } from 'node:stream';
import {
  OBSERVER_IMAGE_SLOTS,
  OBSERVER_RETURN_IMAGE_SLOTS,
...
export async function writeObserverCameraImage(image, cameraId = 'overview', slot = 'latest') {
  const camera = normalizeObserverCameraId(cameraId);
  const normalizedSlot = normalizeObserverSlot(slot);
  if (!camera || !normalizedSlot) throw new Error('Unknown Observer camera or image slot.');
  if (camera === 'return' && normalizedSlot !== 'latest') throw new Error('Return camera supports only the latest image slot.');
  ...
  return putObject(key, body, 'image/jpeg');
}

export async function writeObserverImage(image, slot = 'latest') {
  return writeObserverCameraImage(image, 'overview', slot);
}
...
export async function readObserverCameraImage(cameraId = 'overview', slot = 'latest') {
  const camera = normalizeObserverCameraId(cameraId);
  const normalizedSlot = normalizeObserverSlot(slot);
  if (!camera || !normalizedSlot) return null;
  if (camera === 'return' && normalizedSlot !== 'latest') return null;
  const key = camera === 'return' ? OBSERVER_RETURN_IMAGE_SLOTS.latest : OBSERVER_IMAGE_SLOTS[normalizedSlot];
  return getObject(key);
}

export async function readObserverImage(slot = 'latest') {
  return readObserverCameraImage('overview', slot);
}
...
export async function writeObserverTimelapse(video, slot, cameraId = 'overview') {
  ...
  return putObject(slots[normalizedSlot], body, 'video/mp4');
}

export async function readObserverTimelapse(slot, cameraId = 'overview') {
  const normalizedSlot = normalizeObserverTimelapseSlot(slot);
  const camera = normalizeObserverCameraId(cameraId) || 'overview';
  if (!normalizedSlot) return null;
  const slots = camera === 'return' ? OBSERVER_RETURN_TIMELAPSE_SLOTS : OBSERVER_TIMELAPSE_SLOTS;
  return getObject(slots[normalizedSlot]);
}
