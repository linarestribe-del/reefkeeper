...
If planContext.resolvedIssues says an issue is resolved, treat that as newer and more authoritative than the fixed tank profile. Do not include tasks for resolved issues. In particular, if australianStripyRehomed is resolved, do not include Australian Stripy feeding, sump coverage, rehoming, or posting tasks.
If planContext.activeReefTasks contains active reminders or scheduled reef tasks, integrate the most relevant ones into the 7-day plan and preserve any provided scheduledDay unless there is a strong safety reason to move it. Avoid duplicating the same task in multiple days.
