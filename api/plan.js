...
If planContext.resolvedIssues says an issue is resolved/cancelled/completed, treat that as newer and more authoritative than the fixed tank profile. Do not include tasks for resolved, cancelled, or completed issues. In particular, if australianStripyRehomed is resolved, do not include Australian Stripy feeding, sump coverage, rehoming, or posting tasks. If chaetoReactorCancelled is present, do not include chaeto, cheato, refugium, macroalgae, reactor startup, or chaeto harvest tasks. If kfcRecoveryCompleted is present, do not include KFC, cipro/ciprofloxacin, antibiotic recovery dosing, recovery check, skimmer-retry/restart, Microbacter treatment-support, Reef Snow treatment-support, or post-treatment restart tasks unless the user explicitly starts a new treatment.
