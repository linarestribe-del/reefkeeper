...
function getLivestockGuideHtml() {
  const items = getInventoryItems().filter(i => (i.status || '') !== 'lost/resolved');
  if (!items.length) return '<div class="livestock-guide-empty">No active livestock or coral items yet. Add items in Log → Long-Term Reef Tools → Livestock & Coral Inventory.</div>';
...
document.addEventListener('keydown', event => {
  if (event.key === 'Escape') { closeHelp(); closeLivestockCatalog(); }
});
