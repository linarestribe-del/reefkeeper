// Reef Keeper build: 20260619-fixed3
// Early Safari/PWA compatibility fallback. This runs before the rest of the app
// so AI plan generation can always access completed-history helpers, even if
// an older cached build or inline handler looks for window.getCompletedHistory.
(function reefKeeperEarlyCompletedHistoryFallback(){
  try {
    if (typeof window !== 'undefined') {
      window.REEF_KEEPER_BUILD = '20260619-fixed3';
      if (typeof window.getCompletedHistoryEntries !== 'function') {
        window.getCompletedHistoryEntries = function(){
          try {
            var raw = window.localStorage ? window.localStorage.getItem('reef_completed_history') : '[]';
            var parsed = JSON.parse(raw || '[]');
            return Array.isArray(parsed) ? parsed : [];
          } catch(e) { return []; }
        };
      }
      if (typeof window.setCompletedHistoryEntries !== 'function') {
        window.setCompletedHistoryEntries = function(entries){
          try { if (window.localStorage) window.localStorage.setItem('reef_completed_history', JSON.stringify(Array.isArray(entries) ? entries : [])); } catch(e) {}
        };
      }
      if (typeof window.getCompletedHistory !== 'function') {
        window.getCompletedHistory = function(){ return window.getCompletedHistoryEntries(); };
      }
      if (typeof window.setCompletedHistory !== 'function') {
        window.setCompletedHistory = function(entries){ return window.setCompletedHistoryEntries(entries); };
      }
    }
  } catch(e) {}
})();
...
function isPlanTaskTextAlreadyCompleted(blockKey, taskText) {
  return isTaskTextCompletedRecently(taskText, '', { blockKey, maxAgeDays: 21 });
}
...
// Reef Keeper fixed3 hard global exports.
try {
  window.REEF_KEEPER_BUILD = '20260619-fixed3';
  window.getCompletedHistoryEntries = getCompletedHistoryEntries;
  window.setCompletedHistoryEntries = setCompletedHistoryEntries;
  window.getCompletedHistory = getCompletedHistory;
  window.setCompletedHistory = setCompletedHistory;
  window.getCurrentPlanPromptContext = getCurrentPlanPromptContext;
  window.generateAiDaysOffPlan = generateAiDaysOffPlan;
  window.handleAiPlanButton = handleAiPlanButton;
} catch(e) {}
