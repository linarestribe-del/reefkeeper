...
LIVESTOCK: 2 clownfish (breeding pair, hosting Duncan coral, laying eggs regularly), yellow corgi wrasse, melanurus wrasse, red head Solon wrasse, banggai cardinal, blue chromis, yellow tang, white tail bristle tooth tang, desjardini sailfin tang, orange banded goby + pistol shrimp pair, 2 bubble tip anemones (stationary ~1 year, not hosted by clownfish), 2 Halloween hermit crabs, various small hermit crabs, sand sifting starfish (new, 1-2 months), serpent starfish, 2 fighting conchs, hammer coral (2 surviving colonies), green candy cane coral, purple stylophora (lost), Ricordea mushrooms (stressed, below BTAs — allelopathy issue), Grube's gorgonia, Duncan coral (healthy, clownfish host).
...
function openLivestockCatalog() {
  const logBtn = Array.from(document.querySelectorAll('.nav-btn')).find(btn => {
    const label = btn.querySelector('.nav-label');
    return label && label.textContent.trim().toLowerCase() === 'log';
  });
  showPage('log', logBtn || document.querySelector('.nav-btn'));
  setTimeout(() => {
    const inventoryDetails = Array.from(document.querySelectorAll('.long-term-details')).find(d => {
      const summary = d.querySelector('summary');
      return summary && summary.textContent.toLowerCase().includes('livestock');
    });
    if (inventoryDetails) inventoryDetails.open = true;
    const panel = document.getElementById('livestock-guide-panel');
    if (panel) panel.classList.add('visible');
    renderLivestockGuide();
    const target = inventoryDetails || panel;
...
async function sendChat() {
  ...
  const capturedUpdate = autoCaptureTankUpdateFromChat(text);
...
function renderTrendChart(paramKey = currentTrendParam) {
  ...
  const points = getAllLogsForCharts()
    .map((log, index) => ({ index, date: log.date || '', value: parseFloat(log[param.key]) }))
  ...
  container.innerHTML = `
    ...
    <svg class="trend-svg" viewBox="0 0 ${width} ${height}" role="img" aria-label="${param.label} trend chart">
      ...
      <text x="${padL}" y="${height-10}" font-size="10" font-weight="800" fill="#48cae4">Oldest</text>
      <text x="${width-padR-36}" y="${height-10}" font-size="10" font-weight="800" fill="#48cae4">Newest</text>
    </svg>`;
}
...
const INVENTORY_GUIDE_DEFAULTS = {
  ...
  'inv-goby-shrimp': { scientificName:"Amblyeleotris randalli / Alpheus spp. (verify)", naturalRange:'Indo-Pacific sandy reef slopes and rubble zones', facts:['Classic goby-pistol shrimp symbiosis.', 'The shrimp maintains the burrow while the goby watches for danger.', 'Needs sand/rubble and stable rockwork.'] },
  ...
  'inv-hermits': { scientificName:'Paguristes / Clibanarius / Calcinus spp. (mixed)', naturalRange:'Tropical reef rubble and intertidal zones', facts:['Useful scavengers but can steal shells or bother snails.', 'Halloween hermits are larger and more noticeable cleaners.', 'Provide spare shells to reduce conflict.'] },
  'inv-stars': { scientificName:'Astropecten spp. / Ophioderma or Ophiolepis spp. (verify)', naturalRange:'Tropical sandy bottoms and reef rubble zones', facts:['Sand-sifting stars need mature sand beds with enough microfauna.', 'Serpent stars are effective nocturnal scavengers.', 'Watch for weight loss or shrinking central disc.'] },
...
function defaultInventoryItems() {
  return [
    ...
    { id:'inv-goby-shrimp', name:'Orange banded goby + pistol shrimp', type:'fish', status:'stable', location:'Display', notes:'Paired.' },
    ...
    { id:'inv-hermits', name:'Hermit crabs', type:'invert', status:'stable', location:'Display', notes:'Includes two Halloween hermits and various small hermits.' },
    { id:'inv-stars', name:'Sand sifting starfish and serpent starfish', type:'invert', status:'stable', location:'Display', notes:'' },
    { id:'inv-conchs', name:'Fighting conchs', type:'invert', status:'stable', location:'Display', notes:'Two fighting conchs.' },
...
function getInventoryItems() {
  let items = memoryArray('reef_inventory');
  if (!items.length) items = defaultInventoryItems();
  const normalized = items.map(normalizeInventoryItem);
  try { localStorage.setItem('reef_inventory', JSON.stringify(normalized)); } catch(e) {}
  return normalized;
}

function setInventoryItems(items) {
  try { localStorage.setItem('reef_inventory', JSON.stringify(items.map(normalizeInventoryItem))); } catch(e) {}
}

function resizeInventoryImage(file, maxSize = 1100, quality = 0.82) {
...
async function uploadInventoryPhoto(id, event) {
  ...
  try {
    ...
    if (idx < 0) return;
    items[idx] = { ...items[idx], photoData, photoUpdatedAt:new Date().toISOString(), updatedAt:new Date().toISOString() };
...
function saveInventoryItem() {
  ...
  const items = getInventoryItems();
  const existingIndex = items.findIndex(i => String(i.name || '').toLowerCase() === name.toLowerCase());
  ...
  const entry = normalizeInventoryItem({
    ...
    photoData: pendingInventoryPhotoData || existing.photoData || '',
    photoUpdatedAt: pendingInventoryPhotoData ? new Date().toISOString() : existing.photoUpdatedAt || '',
...
function loadInventoryItemForEdit(id) {
  ...
  pendingInventoryPhotoData = item.photoData || '';
  const preview = document.getElementById('inventory-photo-preview');
  if (preview && item.photoData) {
    preview.innerHTML = `<img src="${item.photoData}" alt="Current livestock photo"><div><strong>Current photo loaded.</strong><br><span style="font-size:12px;color:var(--text-mid);font-weight:700;">Choose a new photo to replace it, or save to keep it.</span></div>`;
    preview.classList.add('visible');
  }
...
function deleteInventoryItem(id) {
  setInventoryItems(getInventoryItems().filter(i => i.id !== id));
  renderInventory();
  renderLivestockGuide();
  showToast('🗑️ Inventory item removed');
}

function removeInventoryPhoto(id) {
  const items = getInventoryItems();
  const idx = items.findIndex(i => String(i.id) === String(id));
  if (idx < 0) return;
  items[idx] = { ...items[idx], photoData:'', photoUpdatedAt:'', updatedAt:new Date().toISOString() };
  setInventoryItems(items);
  renderInventory();
  renderLivestockGuide();
...
function renderInventory() {
  ...
  el.innerHTML = sorted.map(i => {
    ...
    return `<div class="inventory-item">
      <div class="inventory-photo-thumb">${i.photoData ? `<img src="${i.photoData}" alt="${escapeHtml(i.name)} photo">` : '🐠'}</div>
      <div class="inventory-main">
        ...
        <div class="inventory-actions">
          ...
          ${i.photoData ? `<button class="inventory-small-btn danger" onclick="removeInventoryPhoto('${escapeHtml(i.id)}')">Remove photo</button>` : ''}
        </div>
      </div>
...
function renderLivestockGuide() {
  ...
  if (!el) return;
  const items = getInventoryItems().filter(i => (i.status || '') !== 'lost/resolved');
  if (!items.length) {
    el.innerHTML = '<div class="livestock-guide-empty">No active livestock or coral items yet.</div>';
    return;
  }
  ...
  el.innerHTML = sorted.map(i => {
    ...
    return `<article class="livestock-guide-card">
      <div class="livestock-guide-photo">${i.photoData ? `<img src="${i.photoData}" alt="${escapeHtml(i.name)}">` : 'Upload photo'}</div>
