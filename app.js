...
function getInventoryItems() {
  ...
  if (!items.length) {
    ...
    try { localStorage.setItem('reef_inventory', JSON.stringify(items)); } catch(e) {}
  }
  return items;
}
...
function saveInventoryItem() {
  ...
  const entry = { id: existingIndex >= 0 ? items[existingIndex].id : Date.now().toString(36) + Math.random().toString(36).slice(2), name, type, status, location, notes, updatedAt:new Date().toISOString() };
  if (existingIndex >= 0) items[existingIndex] = entry; else items.unshift(entry);
  setInventoryItems(items);
  ['inventory-name','inventory-location','inventory-notes'].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
  renderInventory();
  ...
  el.innerHTML = sorted.map(i => `
    <div class="inventory-item">
      <div class="inventory-main">
        ...
        <div class="inventory-meta">${escapeHtml(i.type || 'other')} · ${escapeHtml(i.status || 'stable')}${i.location ? ` · ${escapeHtml(i.location)}` : ''}</div>
