...
function migrateInventoryItems(items) {
  ...
  const combinedGoby = next.find(i => String(i.id) === 'inv-goby-shrimp' || /goby\s*\+\s*pistol|pistol shrimp/i.test(String(i.name || '')));
  if (combinedGoby) {
...
function getInventoryItems() {
  ...
  const normalized = migrateInventoryItems(items);
...
function setInventoryItems(items) {
  try {
    localStorage.setItem('reef_inventory', JSON.stringify(items.map(normalizeInventoryItem)));
    localStorage.removeItem('reef_inventory_last_error');
    return true;
  } catch(e) {
    try { localStorage.setItem('reef_inventory_last_error', e && e.message ? e.message : 'Could not save inventory'); } catch(_) {}
    console.error('Could not save reef_inventory', e);
    return false;
  }
}

function resizeInventoryImage(file, maxSize = 900, quality = 0.72) {
...
async function uploadInventoryPhoto(id, event) {
  ...
  try {
    ...
    if (!setInventoryItems(items)) {
      showToast('⚠️ Could not save photo. Try a smaller image.');
      return;
    }
    renderInventory();
...
async function fillInventoryWithAI() {
  ...
  try {
    ...
    showToast('✨ AI details filled. Review before saving.');
  } catch(e) {
    showToast('⚠️ Could not fill details with AI');
  } finally {
...
function saveInventoryItem() {
  ...
  if (!saved) {
    showToast('⚠️ Could not save. Try removing a large photo or exporting a backup first.');
    return;
  }
  clearInventoryForm();
  renderInventory();
...
function deleteInventoryItem(id) {
  if (!setInventoryItems(getInventoryItems().filter(i => i.id !== id))) { showToast('⚠️ Could not update inventory'); return; }
  renderInventory();
  renderLivestockGuide();
  renderLivestockCatalogModal();
...
function renderInventory() {
  ...
  el.innerHTML = sorted.map(i => {
    ...
    return `<div class="inventory-item">
      <div class="inventory-photo-thumb">${i.photoData ? `<img src="${i.photoData}" alt="${escapeHtml(i.name)} photo" onerror="this.parentElement.textContent='🐠'">` : '🐠'}</div>
...
function livestockPhotoHtml(item) {
  ...
  return `<div class="livestock-guide-photo"><img src="${item.photoData}" alt="${escapeHtml(item.name)}" onerror="this.parentElement.classList.add('no-photo');this.parentElement.innerHTML='<div>📷</div><span>No photo uploaded</span>'"></div>`;
}
