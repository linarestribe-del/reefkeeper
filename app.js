...
function setInventoryItems(items) {
  const compact = items.map(item => {
    ...
    // Photo data is stored in IndexedDB, not localStorage. This prevents browser storage
    // quota errors that made new livestock disappear after navigating away.
...
async function saveInventoryItem() {
  ...
  const entryId = existing.id || Date.now().toString(36) + Math.random().toString(36).slice(2);
