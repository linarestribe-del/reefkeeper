...
function getTankPhotoTimelineMemoryLines() {
  try {
    if (typeof getTankPhotoTimeline !== 'function') return [];
    const photos = getTankPhotoTimeline();
    if (!Array.isArray(photos) || !photos.length) return [];
    return photos
      .slice(0, 12)
      .map(photo => {
        const date = photo.createdAt ? formatDateShort(photo.createdAt) : 'undated';
        const title = photo.title || 'Tank photo';
        const note = photo.note ? ` - ${photo.note}` : '';
        const source = photo.source ? ` (${photo.source})` : '';
        return compactMemoryLine(`${date}: ${title}${source}${note}`);
      });
  } catch (e) {
    console.warn('Could not build tank photo timeline memory lines', e);
    return [];
  }
}
...
function getLocalTankMemorySummary(userMsg = '') {
  ...
  const photoLines = getTankPhotoTimelineMemoryLines();
...
function openLongTermTool(tool) {
  ...
  if (tool === 'reports') { renderReportTypeGrid(); generateSelectedReportPreview(); }
  if (tool === 'knowledge') { renderTankKnowledgeBase(); }
  if (tool === 'photos') { renderTankPhotoTimeline(); }
...
async function postJsonWithFallback(paths, payload) {
  ...
  throw lastError || new Error('Could not reach Reef Keeper API.');
}
...
async function askOpenAI(userMsg, history, modelMode = getModelMode()) {
  ...
  const data = await postJsonWithFallback(['/api/chat', '/api/chat.js'], {
...
function handleFileUpload(event) {
  return handleDocumentUpload(event);
}
...
function handleAddMenuOverlayClick(event) {
  if (event && event.target && event.target.id === 'add-menu-overlay') closeAddMenu();
}

function chooseAddMenuAction(kind) {
  closeAddMenu();
  const map = {
    camera: 'camera-photo-input',
    library: 'library-photo-input',
    document: 'doc-file-input'
  };
  const input = document.getElementById(map[kind]);
  if (input) input.click();
}

function fileLooksTextReadable(file) {
...
function handleDocumentUpload(event) {
  ...
  if (file.type && file.type.startsWith('image/')) {
    handleTankPhotoFile(file, 'document');
    if (event?.target) event.target.value = '';
    return;
  }

  if (!fileLooksTextReadable(file)) {
    attachedFileContext = {
      name: file.name,
      text: `The user attached a document named "${file.name}" (${file.type || 'unknown file type'}). This web app cannot directly read the contents of this binary document yet. Ask the user to paste key text or upload a text/CSV export if detailed analysis is needed.`
    };
    updateAttachmentStatus();
  ...
  reader.onload = () => {
    ...
    const cleaned = raw.replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F]/g, ' ').slice(0, 8000);
    attachedFileContext = { name: file.name, text: cleaned || 'The selected text file appeared to be empty.' };
    updateAttachmentStatus();
...
function handleTankPhotoQuickAdd(event, source) {
  const file = event && event.target && event.target.files ? event.target.files[0] : null;
  if (!file) return;
  handleTankPhotoFile(file, source || 'photo');
  if (event?.target) event.target.value = '';
}
...
// ── Tank photo timeline ─────────────────────────────────────────────────────
const TANK_PHOTO_TIMELINE_KEY = 'reef_tank_photo_timeline';

function getTankPhotoTimeline() {
  try {
    const value = JSON.parse(localStorage.getItem(TANK_PHOTO_TIMELINE_KEY) || '[]');
    return Array.isArray(value) ? value : [];
  } catch(e) { return []; }
}

function setTankPhotoTimeline(items) {
  try {
    localStorage.setItem(TANK_PHOTO_TIMELINE_KEY, JSON.stringify(Array.isArray(items) ? items : []));
    return true;
  } catch(e) {
    console.error('Could not save tank photo timeline', e);
    return false;
  }
}

function tankPhotoTitleFallback(source) {
  if (source === 'camera') return 'New tank photo';
  if (source === 'library') return 'Selected tank photo';
  return 'Uploaded tank photo';
}

async function handleTankPhotoFile(file, source) {
  if (!file || !file.type || !file.type.startsWith('image/')) {
    showToast('⚠️ Please choose an image');
    return;
  }
  try {
    const titleEl = document.getElementById('tank-photo-title');
    const noteEl = document.getElementById('tank-photo-note');
    const title = (titleEl?.value || '').trim() || tankPhotoTitleFallback(source);
    const note = (noteEl?.value || '').trim();
    const id = `tank-photo-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 7)}`;
    const photoKey = `${id}-image`;
    const dataUrl = await resizeImageFile(file, 1200, 0.82);
    await saveInventoryPhotoData(photoKey, dataUrl);
    const items = getTankPhotoTimeline();
    items.unshift({
      id,
      photoKey,
      title,
    ...
    if (!setTankPhotoTimeline(items)) throw new Error('Could not save photo metadata');
    if (titleEl) titleEl.value = '';
    if (noteEl) noteEl.value = '';
    renderTankPhotoTimeline();
    showToast('📸 Photo saved to timeline');
    if (document.getElementById('tool-photos-overlay')) {
      document.getElementById('tool-photos-overlay').classList.add('visible');
    }
  } catch(e) {
    console.error('Tank photo save failed:', e);
    showToast('⚠️ Could not save photo');
  }
}

async function renderTankPhotoTimeline() {
  const list = document.getElementById('tank-photo-list');
  if (!list) return;
  const items = getTankPhotoTimeline();
  if (!items.length) {
    list.innerHTML = '<div class="livestock-guide-empty">No tank photos saved yet. Use Take Photo or Choose Photo to start a visual timeline.</div>';
    return;
  }
  list.innerHTML = items.map(item => `
    <div class="tank-photo-card" data-photo-id="${escapeHtml(item.id)}">
      <div class="tank-photo-loading">Loading photo…</div>
      <div class="tank-photo-title">${escapeHtml(item.title || 'Tank photo')}</div>
      <div class="tank-photo-meta">${escapeHtml(formatDateTimeShort(item.createdAt))}${item.fileName ? ' · ' + escapeHtml(item.fileName) : ''}</div>
      ${item.note ? `<div class="tank-photo-note">${escapeHtml(item.note)}</div>` : ''}
      <button class="tank-photo-delete" type="button" onclick="deleteTankPhoto('${escapeHtml(item.id)}')">Delete photo</button>
    </div>`).join('');
  await hydrateTankPhotoTimelineImages(list, items);
}

async function hydrateTankPhotoTimelineImages(root, items) {
  for (const item of items) {
    const card = root.querySelector(`[data-photo-id="${cssEscapeSafe(item.id)}"]`);
    if (!card) continue;
    const data = await getInventoryPhotoData(item.photoKey);
    const holder = card.querySelector('.tank-photo-loading');
    if (!holder) continue;
    if (data) {
      holder.outerHTML = `<img class="tank-photo-img" alt="${escapeHtml(item.title || 'Tank photo')}" src="${data}">`;
    } else {
      holder.outerHTML = '<div class="livestock-guide-photo no-photo">📷<span>Photo unavailable</span></div>';
    }
  }
}

async function deleteTankPhoto(id) {
  const items = getTankPhotoTimeline();
  const item = items.find(p => p.id === id);
  if (!item) return;
  if (!confirm('Delete this tank photo from the timeline?')) return;
  await deleteInventoryPhotoData(item.photoKey);
  setTankPhotoTimeline(items.filter(p => p.id !== id));
  renderTankPhotoTimeline();
  showToast('🗑️ Tank photo deleted');
}

function formatDateTimeShort(value) {
  const d = new Date(value || Date.now());
  if (Number.isNaN(d.getTime())) return String(value || 'Recent');
...
function cssEscapeSafe(value) {
  try { return (window.CSS && CSS.escape) ? CSS.escape(String(value || '')) : String(value || '').replace(/[^a-zA-Z0-9_-]/g, '\\$&'); }
...
async function sendChat(event) {
  ...
  try {
    ...
    removeTyping();
    const detail = e && e.message ? String(e.message).slice(0, 220) : 'Unknown connection error';
    appendMsg('ai', `⚠️ Couldn\'t connect to AI. ${detail}`);
    scrollChatToBottom();
  }
}
...
// ── Report Center ───────────────────────────────────────────────────────────
const REPORT_TYPES = [
  { id:'monthly', name:'Monthly Reef Report', desc:'Status summary, trends, risks, recent changes, and next focus.' },
  { id:'livestock', name:'Livestock Catalog PDF', desc:'Printable fish, invert, coral, and anemone catalog from your inventory.' },
  { id:'timeline', name:'Tank Timeline PDF', desc:'Chronological tank history from logs, actions, completed tasks, and knowledge notes.' },
  { id:'equipment', name:'Equipment Guide', desc:'Tank overview, equipment list, maintenance rhythm, and replacement notes.' },
  { id:'emergency', name:'Emergency Binder', desc:'Printable emergency guide for overheating, power loss, salinity, chemistry, and livestock issues.' },
  { id:'custom', name:'Custom Report', desc:'Describe the report you want, and Reef Keeper builds it from stored tank data.' }
];
let currentReportType = 'monthly';
let currentReportHtml = '';
let currentReportTitle = 'Reef Keeper Report';

function renderReportTypeGrid() {
  const grid = document.getElementById('report-type-grid');
  if (!grid) return;
  grid.innerHTML = REPORT_TYPES.map(r => `
    <button class="report-type-card${r.id === currentReportType ? ' active' : ''}" type="button" onclick="setReportType('${r.id}')">
      <div class="report-type-name">${escapeHtml(r.name)}</div>
      <div class="report-type-desc">${escapeHtml(r.desc)}</div>
    </button>`).join('');
  const custom = document.getElementById('report-custom-prompt');
  if (custom) custom.classList.toggle('visible', currentReportType === 'custom');
}

function setReportType(type) {
  currentReportType = REPORT_TYPES.some(r => r.id === type) ? type : 'monthly';
  renderReportTypeGrid();
  const preview = document.getElementById('report-preview');
  if (preview) preview.innerHTML = 'Tap <strong>Generate Report</strong> to build this document.';
}
...
function reportSlug(title) {
  return String(title || 'reef-keeper-report').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 80) || 'reef-keeper-report';
}

function reportSection(title, body) {
  return `<h2>${escapeHtml(title)}</h2>${body || '<p class="report-muted">No data yet.</p>'}`;
}

function reportList(items) {
  const clean = (items || []).filter(Boolean);
  if (!clean.length) return '<p class="report-muted">No entries yet.</p>';
  return `<ul>${clean.map(i => `<li>${i}</li>`).join('')}</ul>`;
}

function reportShell(title, innerHtml) {
  currentReportTitle = title;
  return `
    <h1>${escapeHtml(title)}</h1>
    <div class="report-muted">Generated by Reef Keeper · ${escapeHtml(reportFullDate())}</div>
    ${innerHtml}`;
}

function getLatestReportValues() {
  const latest = {};
  TREND_PARAMS.forEach(p => {
    const found = getLatestValueForKey(p.key);
    if (found && found[p.key] !== undefined && found[p.key] !== '') latest[p.key] = { label:p.label, value:found[p.key], unit:p.unit || '', date:memoryLineDate(found) };
  });
  return latest;
}

function latestParameterHtml() {
  const latest = getLatestReportValues();
  const items = Object.values(latest).map(p => `${escapeHtml(p.label)}: <strong>${escapeHtml(p.value)} ${escapeHtml(p.unit)}</strong> <span class="report-muted">(${escapeHtml(p.date)})</span>`);
  return reportList(items);
}

function trendSummaryHtml() {
  const lines = buildParameterTrendSummary(getAllLogsForCharts()).split('\n').filter(Boolean).map(escapeHtml);
  return reportList(lines);
}

function recentChangesHtml(limit = 12) {
  const changes = getRecentChanges(limit).map(c => `${escapeHtml(c.meta)} — ${escapeHtml(c.title)}`);
  return reportList(changes);
}

function knowledgeHtml(limit = 12) {
  const items = getTankKnowledgeItems().slice(0, limit).map(k => `<strong>${escapeHtml(k.category)}: ${escapeHtml(k.title)}</strong> — ${escapeHtml(k.note)}`);
  return reportList(items);
}

function guardrailsHtml() {
  return reportList(getGuardrails().map(g => escapeHtml(g.text)));
}

function maintenanceIntervalsHtml() {
  return reportList(defaultMaintenanceIntervals().map(escapeHtml));
}

function inventorySummaryHtml() {
  const items = getInventoryItems();
  if (!items.length) return '<p class="report-muted">No livestock inventory items yet.</p>';
  const grouped = { fish:[], invert:[], coral:[], other:[] };
  items.forEach(i => {
    const group = inventoryTabGroup(i);
    (grouped[group] || grouped.other).push(i);
  });
  return Object.entries(grouped).filter(([, arr]) => arr.length).map(([group, arr]) => `
    <h3>${escapeHtml(group === 'invert' ? 'Inverts' : group.charAt(0).toUpperCase()+group.slice(1))}</h3>
    ${reportList(arr.map(i => `<strong>${escapeHtml(i.name)}</strong>${i.scientific ? ` <em>${escapeHtml(i.scientific)}</em>` : ''} — ${escapeHtml(i.status || 'status not set')}${i.location ? ` · ${escapeHtml(i.location)}` : ''}${i.notes ? ` · ${escapeHtml(i.notes)}` : ''}`))}
  `).join('');
}

async function livestockCatalogReportHtml() {
  const items = getInventoryItems();
  if (!items.length) return reportShell('Livestock Catalog', '<p class="report-muted">No livestock inventory items yet.</p>');
  const sorted = [...items].sort((a,b) => inventoryTabGroup(a).localeCompare(inventoryTabGroup(b)) || String(a.name).localeCompare(String(b.name)));
  const cards = [];
  for (const item of sorted) {
    ...
    if (item.photoKey) {
      try {
        const data = await getInventoryPhotoData(item.photoKey);
        if (data) img = `<img class="report-photo" src="${data}" alt="${escapeHtml(item.name)}">`;
      } catch(e) {}
    }
    const facts = String(item.facts || '').split(/\n+/).map(s => s.trim()).filter(Boolean).map(escapeHtml);
    cards.push(`
      <h2>${escapeHtml(item.name || 'Livestock item')}</h2>
      ${img || '<p class="report-muted">No photo uploaded.</p>'}
      ${item.scientific ? `<p><em>${escapeHtml(item.scientific)}</em></p>` : ''}
      ${item.range ? `<p><strong>Natural range:</strong> ${escapeHtml(item.range)}</p>` : ''}
      ${item.location ? `<p><strong>Tank location:</strong> ${escapeHtml(item.location)}</p>` : ''}
      ${item.status ? `<p><strong>Status:</strong> ${escapeHtml(item.status)}</p>` : ''}
      ${facts.length ? `<h3>Facts</h3>${reportList(facts)}` : ''}
      ${item.notes ? `<h3>Your notes</h3><p>${escapeHtml(item.notes)}</p>` : ''}
    `);
  }
  return reportShell('Livestock Catalog', cards.join(''));
}

function buildTankTimelineItems(limit = 120) {
  const items = [];
  getAllLogsForCharts().forEach(l => items.push({ date:memoryDateValue(l), label:memoryLineDate(l), type:'Parameter log', text:buildLogMemoryLine(l) }));
  getActionEntries().forEach(a => items.push({ date:memoryDateValue(a), label:memoryLineDate(a), type:a.category || 'Action', text:buildActionMemoryLine(a) }));
  getCompletedHistoryEntries().forEach(h => items.push({ date:memoryDateValue(h), label:memoryLineDate(h), type:h.type || 'Completed', text:buildCompletedMemoryLine(h) }));
  getTankKnowledgeItems().forEach(k => items.push({ date:memoryDateValue({ createdAt:k.createdAt }), label:reportDateLabel(k.createdAt), type:'Knowledge', text:`${k.category}: ${k.title} — ${k.note}` }));
  getInventoryItems().filter(i => i.createdAt).forEach(i => items.push({ date:memoryDateValue({ createdAt:i.createdAt }), label:reportDateLabel(i.createdAt), type:'Inventory', text:`${i.name} added/updated in inventory${i.status ? ` (${i.status})` : ''}` }));
  return items.sort((a,b) => b.date - a.date).slice(0, limit);
}

function timelineHtml(limit = 80) {
  const items = buildTankTimelineItems(limit).map(i => `<strong>${escapeHtml(i.label)} · ${escapeHtml(i.type)}</strong> — ${escapeHtml(i.text)}`);
  return reportList(items);
}

function equipmentGuideHtml() {
  const equipment = [
    '120 gallon SCA display with 50 gallon Red Sea Reefer sump.',
    'Two Jebao MDP Smart DC return pumps.',
    'Two Hygger 802 titanium heaters.',
    'Bubble Magus filter roller.',
    'Simplicity 240 DC protein skimmer with outdoor air intake.',
    '27W IceCap UV sterilizer.',
    'IceCap 120 GFO reactor and two-stage media reactor for GFO / ROX 0.8 carbon.',
    'Four A8se 11 Max lights.',
    'Two MP40 powerheads and one Jebao DMP20.',
    'Useek smart ATO with 10 gallon reservoir.',
    'Neptune Apex controller.',
    '5-stage RODI with booster pump.',
    '55 gallon Brute saltwater mixing can.',
    'DIY kalk stirrer present, but kalk dosing is on hold by current guardrail.'
  ];
  return reportShell('Equipment Guide',
    reportSection('System overview', `<p>${escapeHtml('120 gallon mixed reef system with sump, automation, UV, media filtration, protein skimming, and planned conservative long-term maintenance routines.')}</p>`) +
    reportSection('Equipment list', reportList(equipment.map(escapeHtml))) +
    reportSection('Maintenance intervals', maintenanceIntervalsHtml()) +
    reportSection('Guardrails', guardrailsHtml())
  );
}

function emergencyBinderHtml() {
  const emergency = [
    '<strong>Power outage:</strong> prioritize water movement and oxygen. Run battery air pumps or powerheads if available. Keep temperature stable and avoid feeding heavily.',
    '<strong>Overheating:</strong> verify heaters/Apex state, use fans, float sealed ice bottles if needed, and lower temperature gradually. Log the event.',
    '<strong>Low salinity:</strong> verify with calibrated refractometer, correct slowly with saltwater top-off or small adjusted water changes.',
    '<strong>High salinity:</strong> verify reading, correct slowly with RODI replacement. Avoid rapid swings.',
    '<strong>Alkalinity spike/drop:</strong> stop dosing changes, retest, check recent water changes/dosing, and avoid adding another intervention until confirmed.',
    '<strong>High phosphate:</strong> do not crash phosphate. Use gradual export and monitor coral response.',
    '<strong>Coral stress:</strong> check temperature, salinity, alkalinity, recent media changes, pest treatments, and allelopathy. Use carbon if chemical warfare is suspected.',
    '<strong>Fish distress:</strong> check oxygen/flow, temperature, salinity, ammonia concern, aggression, and disease signs. Avoid overmedicating without identification.',
    '<strong>Emergency principle:</strong> change one thing at a time when possible, document the event, and favor stability over chasing numbers.'
  ];
  return reportShell('Reef Emergency Binder',
    reportSection('Tank snapshot', `<p>${escapeHtml('120 gallon SCA display + 50 gallon sump. Reef Keeper tank context and guardrails should be checked before major changes.')}</p>`) +
    reportSection('Emergency procedures', reportList(emergency)) +
    reportSection('Current guardrails', guardrailsHtml()) +
    reportSection('Latest parameters', latestParameterHtml()) +
    reportSection('Livestock overview', inventorySummaryHtml())
  );
}

function monthlyReportHtml() {
  return reportShell('Monthly Reef Report',
    reportSection('Executive summary', `<p>${escapeHtml(buildLongTermTankSummary(getAllLogsForCharts(), getActionEntries(), getCompletedHistoryEntries()))}</p>`) +
    reportSection('Latest parameters', latestParameterHtml()) +
    reportSection('Trend summary', trendSummaryHtml()) +
    reportSection('What changed recently', recentChangesHtml(12)) +
    reportSection('Livestock and coral notes', inventorySummaryHtml()) +
    reportSection('Tank knowledge and guardrails', knowledgeHtml(10) + guardrailsHtml()) +
    reportSection('Recommended focus', reportList([
      'Keep changes spaced out and avoid stacking major interventions on the same day.',
      'Use trends and coral response before increasing export or dosing changes.',
      'Keep logging maintenance actions before asking AI for next-step decisions.'
    ]))
  );
}

function timelineReportHtml() {
  return reportShell('Tank Timeline Report',
    reportSection('Chronological history', timelineHtml(100)) +
    reportSection('Trend summary', trendSummaryHtml()) +
    reportSection('Knowledge base decisions', knowledgeHtml(20))
  );
}

function customReportHtml() {
  const prompt = String(document.getElementById('report-custom-prompt')?.value || '').trim();
  return reportShell(prompt ? `Custom Report: ${prompt}` : 'Custom Reef Report',
    reportSection('Custom focus', `<p>${escapeHtml(prompt || 'No custom focus entered. This report includes the broad tank snapshot.')}</p>`) +
    reportSection('Latest parameters', latestParameterHtml()) +
    reportSection('Trend summary', trendSummaryHtml()) +
    reportSection('Recent timeline', timelineHtml(35)) +
    reportSection('Livestock and coral notes', inventorySummaryHtml()) +
    reportSection('Knowledge base and guardrails', knowledgeHtml(15) + guardrailsHtml())
  );
}

async function buildReportHtml(type = currentReportType) {
  if (type === 'livestock') return await livestockCatalogReportHtml();
  if (type === 'timeline') return timelineReportHtml();
  if (type === 'equipment') return equipmentGuideHtml();
  if (type === 'emergency') return emergencyBinderHtml();
  if (type === 'custom') return customReportHtml();
  return monthlyReportHtml();
}

async function generateSelectedReportPreview() {
  const preview = document.getElementById('report-preview');
  if (!preview) return;
  preview.innerHTML = '<p class="report-muted">Generating report…</p>';
  try {
    currentReportHtml = await buildReportHtml(currentReportType);
    preview.innerHTML = currentReportHtml;
    showToast('✅ Report generated');
  } catch(e) {
    console.error(e);
    currentReportHtml = '';
    preview.innerHTML = '<p class="report-muted">Could not generate this report. Try again after refreshing the app.</p>';
  }
}

function fullStandaloneReportHtml() {
  const body = currentReportHtml || document.getElementById('report-preview')?.innerHTML || '';
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${escapeHtml(currentReportTitle)}</title><style>
    body{font-family:Arial,sans-serif;line-height:1.55;color:#123;margin:32px;max-width:820px} h1{font-size:28px;margin-bottom:4px} h2{margin-top:24px;color:#0077b6;border-bottom:1px solid #d8eef6;padding-bottom:4px} h3{margin-top:16px} ul{padding-left:22px} li{margin:5px 0}.report-muted{color:#6d99aa;font-size:12px;font-weight:bold}.report-photo{max-width:100%;max-height:380px;object-fit:cover;border-radius:12px;margin:10px 0} @media print{body{margin:20mm}.report-photo{max-height:260px} h2{break-after:avoid}}
  </style></head><body>${body}</body></html>`;
}

function ensureReportGenerated() {
  if (currentReportHtml && !/Choose a report type|Tap <strong>Generate Report/.test(currentReportHtml)) return true;
  showToast('Generate the report first');
  return false;
}

function printCurrentReport() {
  if (!currentReportHtml) { generateSelectedReportPreview().then(printCurrentReport); return; }
  const win = window.open('', '_blank');
  if (!win) { showToast('⚠️ Popup blocked. Allow popups to print.'); return; }
  win.document.open();
  win.document.write(fullStandaloneReportHtml());
  win.document.close();
...
function downloadCurrentReportHtml() {
  if (!currentReportHtml) { generateSelectedReportPreview().then(downloadCurrentReportHtml); return; }
  downloadTextFile(`${reportSlug(currentReportTitle)}.html`, fullStandaloneReportHtml(), 'text/html;charset=utf-8');
}

function downloadCurrentReportDoc() {
  if (!currentReportHtml) { generateSelectedReportPreview().then(downloadCurrentReportDoc); return; }
  downloadTextFile(`${reportSlug(currentReportTitle)}.doc`, fullStandaloneReportHtml(), 'application/msword;charset=utf-8');
}
...
document.addEventListener('keydown', event => {
  if (event.key === 'Escape') { closeHelp(); closeChatHistory(); closeLivestockCatalog(); closeLongTermTool('inventory'); closeLongTermTool('strategy'); closeLongTermTool('summary'); closeLongTermTool('reports'); closeLongTermTool('knowledge'); }
});
