// Reef Keeper build marker: 20260619-fixed5-hidden-plan-repair
window.REEF_KEEPER_BUILD = '20260619-fixed5-hidden-plan-repair';
...
function shouldFilterResolvedPlanTask(text) {
  ...
  if (isHammerTorchLost() && isHammerTorchCareTask(value)) return true;
  return false;
}
...
function autoCaptureTankUpdateFromChat(text) {
  ...
  if (mentionsKfc && completedKfc) {
    applyKfcRecoveryCompletedSideEffects();
    return { key: 'kfcRecoveryCompleted', label: 'KFC recovery protocol marked completed' };
  }
  const mentionsHammerTorch = /(hammer|hammers|torch|torches|euphyllia)/.test(normalized);
  const lostHammerTorch = /(lost|gone|dead|died|removed|no more|none left|all gone|all lost)/.test(normalized);
  if (mentionsHammerTorch && lostHammerTorch) {
    applyHammersTorchesLostSideEffects();
    return { key: 'hammersTorchesLost', label: 'Hammer/torch corals marked lost/resolved' };
  }
