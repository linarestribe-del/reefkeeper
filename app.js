...
function getCustomInventoryItems() {
  try {
    const raw = JSON.parse(localStorage.getItem(REEF_CUSTOM_INVENTORY_KEY) || '[]');
    return Array.isArray(raw) ? raw.map(normalizeInventoryItem) : [];
  } catch(e) { return []; }
}

function setCustomInventoryItems(items) {
  try {
    localStorage.setItem(REEF_CUSTOM_INVENTORY_KEY, JSON.stringify(compactInventoryItems(items)));
    return true;
  } catch(e) {
    console.error('Could not save custom reef inventory', e);
    return false;
  }
}
...
function persistCustomInventorySubset(items) {
  ...
  const custom = (items || []).filter(item => !defaultIds.has(String(item.id || '')));
...
function getInventoryItems() {
  ...
  const normalized = migrateInventoryItems(mergeInventoryItems(items, customItems));
  try { localStorage.setItem('reef_inventory', JSON.stringify(compactInventoryItems(normalized))); } catch(e) {}
  persistCustomInventorySubset(normalized);
  return normalized;
}
...
function setInventoryItems(items) {
  ...
  try {
    localStorage.setItem('reef_inventory', JSON.stringify(compact));
    persistCustomInventorySubset(compact);
    ...
    const customSaved = setCustomInventoryItems(compact.filter(item => !defaultInventoryIdSet().has(String(item.id || ''))));
    return customSaved;
  }
}
...
async function saveInventoryItem() {
  ...
  if (existingIndex < 0) existingIndex = items.findIndex(i => String(i.name || '').toLowerCase() === name.toLowerCase());
