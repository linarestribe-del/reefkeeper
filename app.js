// Reef Keeper build marker: 20260619-fixed4-kfc-history-reset
window.REEF_KEEPER_BUILD = '20260619-fixed4-kfc-history-reset';
...
function applyKfcRecoveryCompletedSideEffects(options = {}) {
  ...
  getCurrentDaysOffPlan().days.forEach(day => {
    day.tasks.forEach((task, idx) => {
      if (isKfcRecoveryText(`${day.title} ${task}`)) hiddenPlan.add(`d${day.day}-t${idx}`);
    });
  });
  setHiddenPlanTasks(Array.from(hiddenPlan));
