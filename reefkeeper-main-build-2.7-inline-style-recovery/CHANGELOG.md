## Unreleased – Build 2.7: Inline Style Recovery

- Embedded the complete application stylesheet directly in `index.html` so the interface cannot fall back to unstyled HTML when nested CSS files are omitted during deployment.
- Preserved the native Why disclosure and updated its visible marker to Build 2.7.
- Added `app-build-2-7.css` at the project root as a diagnostic backup.
