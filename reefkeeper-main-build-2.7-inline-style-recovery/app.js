...
async function askOpenAI(userMsg, history, modelMode = getModelMode()) {
  ...
  try {
    window.__reefLastStructuredEvidenceContext = null;
    window.__reefLastDecisionReview = null;
    window.__reefLastDecisionReviewError = '';
  } catch(e) {}
  ...
  if (useTankContext && window.ReefKeeperAIContext) {
    try {
      ...
      try { window.__reefLastDecisionReviewError = error && error.message ? error.message : 'Structured review unavailable.'; } catch(e) {}
    }
  }
...
function appendMsg(role, text, options = {}) {
  ...
  const whyMarkup = role === 'ai' && options.showWhy
    ? '<div class="ai-why-wrap"><button class="ai-why-btn" type="button" aria-expanded="false">Why?</button><div class="ai-why-panel"><div><strong>Build:</strong> 2.4</div></div></div>'
    : '';
  div.innerHTML = `${avatar}<div class="msg-bubble">${String(text || '').replace(/\n/g, '<br>')}${whyMarkup}</div>`;
  msgs.appendChild(div);

  if (role === 'ai' && options.showWhy) {
    const button = div.querySelector('.ai-why-btn');
    const panel = div.querySelector('.ai-why-panel');
    if (panel) panel.innerHTML = `<div><strong>Build:</strong> 2.4</div>${buildAIWhyDetailsHtml()}`;
    if (button && panel) {
...
function safeAIWhyList(value, limit = 3) {
  return Array.isArray(value) ? value.slice(0, limit) : [];
}

function buildAIWhyDetailsHtml() {
  try {
    const review = window.__reefLastDecisionReview || null;
    const context = window.__reefLastStructuredEvidenceContext || null;
    const error = window.__reefLastDecisionReviewError || '';

    if (!review || !context) {
      return `<div><strong>Tank context:</strong> Enabled</div><div style="margin-top:6px">This answer used the available tank context. A complete structured confidence review was not available for this response.</div>${error ? `<div style="margin-top:6px"><strong>Review note:</strong> ${escapeHtml(error)}</div>` : ''}<div style="margin-top:6px">Verify current measurements before making dosing or equipment changes.</div>`;
    }

    const confidence = review && typeof review.confidence === 'object' ? review.confidence : {};
    const rawEvidence = Array.isArray(context.evidence) ? context.evidence : [];
    const evidence = rawEvidence.slice().sort((a, b) => Number(b?.effectiveWeight || 0) - Number(a?.effectiveWeight || 0)).slice(0, 4);
    const missing = safeAIWhyList(review.missingOrStaleData, 3);
    const concerns = safeAIWhyList(review?.skepticReview?.concerns, 3);
    const score = Number.isFinite(Number(confidence.score)) ? Math.round(Number(confidence.score)) : '—';
    const label = confidence.label || 'unrated';
    const ceiling = String(review?.decisionPolicy?.actionCeiling || 'not set').replace(/_/g, ' ');

    return `<div><strong>Confidence:</strong> ${escapeHtml(score)}% (${escapeHtml(label)})</div>${evidence.length ? `<div style="margin-top:6px"><strong>Strongest evidence</strong>${evidence.map(e => `<div>• ${escapeHtml(e?.claim || e?.summary || e?.type || 'Evidence')} (${Math.round(Number(e?.effectiveWeight || 0) * 100)}%)</div>`).join('')}</div>` : ''}${missing.length ? `<div style="margin-top:6px"><strong>Missing or stale</strong>${missing.map(e => `<div>• ${escapeHtml(e?.reason || e?.label || e?.metric || 'Additional current data needed')}</div>`).join('')}</div>` : ''}${concerns.length ? `<div style="margin-top:6px"><strong>Skeptic check</strong>${concerns.map(e => `<div>• ${escapeHtml(e)}</div>`).join('')}</div>` : ''}<div style="margin-top:6px"><strong>Action ceiling:</strong> ${escapeHtml(ceiling)}</div>`;
  } catch (error) {
    console.warn('Could not build Why explanation:', error);
    return `<div><strong>Tank context:</strong> Enabled</div><div style="margin-top:6px">The explanation details could not be fully formatted, but this answer used your tank context.</div><div style="margin-top:6px">Verify current measurements before making dosing or equipment changes.</div>`;
  }
}

function attachAIWhy(messageEl, tankContextWasUsed = true) {
  if (!messageEl || !tankContextWasUsed) return;
  const bubble = messageEl.querySelector('.msg-bubble');
  if (!bubble || bubble.querySelector('.ai-why-btn')) return;

  // Insert the control first so malformed evidence can never make it disappear.
  const wrapper = document.createElement('div');
  wrapper.className = 'ai-why-wrap';
  wrapper.innerHTML = `<button class="ai-why-btn" type="button" aria-expanded="false">Why?</button><div class="ai-why-panel"></div>`;
  bubble.appendChild(wrapper);

  const button = wrapper.querySelector('.ai-why-btn');
  const panel = wrapper.querySelector('.ai-why-panel');
  if (panel) panel.innerHTML = buildAIWhyDetailsHtml();
  if (button && panel) {
    button.addEventListener('click', () => {
      const open = panel.classList.toggle('visible');
      button.setAttribute('aria-expanded', open ? 'true' : 'false');
...
async function sendChat(event) {
  ...
  const tankContextWasUsed = getUseTankContext();
  showTyping();
  scrollChatToBottom();
  ...
  try {
    ...
    removeTyping();
    appendMsg('ai', result.answer || 'I received your question, but the answer came back empty.', { showWhy: tankContextWasUsed });
...
function toggleTrendWhy() {
  const panel = document.getElementById('trend-why-panel');
...
function renderTrendChart(paramKey = currentTrendParam) {
  ...
  const rawPoints = getAllLogsForCharts()
    .map((log, index) => ({ index, date: log.date || '', isoDate: log.isoDate || log.date || '', shortDate: shortTrendDate(log), value: parseFloat(log[param.key]) }))
  ...
  const insight = window.ReefKeeperTrendEngine
    ? window.ReefKeeperTrendEngine.analyze({ paramKey, points: rawPoints, events: getTrendEvents() })
    : null;
  const points = insight?.points?.map((p, index) => ({...p, index, shortDate: new Date(p.time).toLocaleDateString('en-US',{month:'short',day:'numeric'})})) || rawPoints.slice(-12);
  ...
  const firstTime = points[0].time || new Date(points[0].isoDate || points[0].date).getTime();
  const lastTime = points[points.length-1].time || new Date(points[points.length-1].isoDate || points[points.length-1].date).getTime();
  const xTime = t => padL + ((t-firstTime)/Math.max(1,lastTime-firstTime))*(width-padL-padR);
  const x = i => points[i].time ? xTime(points[i].time) : padL + (i/(points.length-1))*(width-padL-padR);
  const y = v => padT + ((maxVal-v)/(maxVal-minVal))*(height-padT-padB);
  const path = points.map((p,i)=>`${i===0?'M':'L'} ${x(i).toFixed(1)} ${y(p.value).toFixed(1)}`).join(' ');
  const latest = points[points.length-1];
  const subtitle = insight ? `${insight.status[0].toUpperCase()+insight.status.slice(1)} · Latest ${latest.value}${param.unit?' '+param.unit:''}` : `Latest: ${latest.value}${param.unit?' '+param.unit:''}`;
  const decimals = param.key==='sal'?3:(param.key==='po4'||param.key==='ph'?2:1);
  const eventSvg = (insight?.events||[]).slice(-6).map(e=>{
    const ex=xTime(e.time);
    return `<line class="trend-event-line" x1="${ex.toFixed(1)}" y1="${padT}" x2="${ex.toFixed(1)}" y2="${height-padB}"/><circle class="trend-event-dot" cx="${ex.toFixed(1)}" cy="${padT+3}" r="3"><title>${escapeHtml(e.title)}</title></circle>`;
  }).join('');
  const eventChips = (insight?.events||[]).slice(-4).map(e=>`<span class="trend-event-chip">${escapeHtml(new Date(e.time).toLocaleDateString('en-US',{month:'short',day:'numeric'}))}: ${escapeHtml(e.title)}</span>`).join('');
  const projection = insight?.projection ? ` At the current measured rate, it may reach ${insight.projection.target} ${param.unit||''} in about ${insight.projection.days} days; treat this as a directional estimate, not a dosing target.` : '';
  const why = (insight?.why||[]).map(v=>`<div>• ${escapeHtml(v)}</div>`).join('');
  ...
  container.innerHTML = `
    ...
    <svg class="trend-svg" viewBox="0 0 ${width} ${height}" role="img" aria-label="${param.label} trend chart">
      ...
      <text x="4" y="${padT+4}" font-size="10" font-weight="800" fill="#0077b6">${maxValRaw.toFixed(decimals)}</text>
      <text x="4" y="${height-padB+4}" font-size="10" font-weight="800" fill="#0077b6">${minValRaw.toFixed(decimals)}</text>
      ${eventSvg}
      <path d="${path}" fill="none" stroke="#0077b6" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" />
      ${points.map((p,i)=>`<circle cx="${x(i).toFixed(1)}" cy="${y(p.value).toFixed(1)}" r="5" fill="#ffffff" stroke="#0077b6" stroke-width="3"><title>${escapeHtml(p.date||new Date(p.time).toLocaleDateString())}: ${p.value}</title></circle>`).join('')}
      ${trendXAxisLabels(points,x,height-10)}
    </svg>
    ${insight?`<div class="trend-insight-card"><div class="trend-insight-title">Reef Keeper trend analysis</div><div class="trend-insight-text">${escapeHtml(insight.summary+projection)}</div>${eventChips?`<div class="trend-event-list">${eventChips}</div>`:''}<button class="trend-why-btn" type="button" onclick="toggleTrendWhy()">Why?</button><div class="trend-why-panel" id="trend-why-panel">${why}</div></div>`:''}`;
}
...
const REEF_BACKUP_KEYS = [
  ...
  'reef_hidden_static_reminders',
  'reef_hidden_plan_tasks',
  'reef_ai_days_off_plans',
  'reef_task_schedule',
  'reef_resolved_issues',
  'reef_model_mode',
  'reef_use_tank_context',
  'reef_tank_mode',
  'reef_inventory',
  'reef_inventory_custom',
  'reef_guardrails',
  'reef_monthly_reviews',
  'reef_inventory_custom_v2',
  'reef_chat_conversations',
